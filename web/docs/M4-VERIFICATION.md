...
| `npm test` (vitest) | **168 passed** in 11 files, including the 13 new RPC tests in `src/core/client.test.ts` and 4 new async-destination tests |
| `npm run build` | clean, `tsc -b` included |
| e2e, real core (21 specs) | **15 passed, 6 skipped**, 0 failed — the 6 are the MOCK-only groups, which skip when a wasm build is present |
| e2e, MOCK core (wasm moved aside) | **6 passed, 15 skipped**, 0 failed |
