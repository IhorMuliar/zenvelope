...
import { STATUS_POLL_MS, isFinalStatus } from "./oneclick";
...
export function solanaExitReducer(
  ...
  switch (event.type) {
    ...
    case "refund":
      return { ...state, refundOverride: event.value, error: null };
...
export function canQuote(state: SolanaExitState): boolean {
  return state.phase === "destination" && !state.busy && state.asset !== null && destinationReady(state);
}
